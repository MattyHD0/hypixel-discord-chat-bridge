...
class RunStillClassAvgCommand extends MinecraftCommand {
  ...
  override async execute(player: string, message: string): Promise<void> {
    try {
      ...
      // Get raw dungeon class experience from raw API data
      ...
      // Use raw experience from API for accurate calculation
  ...
  async getAdditionalBoost(): Promise<number> {
    ...
    // Check by display name only - Derpy gives 50% fishing boost
